...
class EarlyStopping:
    ...
    def save_checkpoint(self, AUPRC, model):
        ...
        # torch.save(model.state_dict(), os.path.join("./Model/", 'Transformer_esm_GraphCAI_EC_contrastive_500.pkl'))
        torch.save(model.state_dict(), os.path.join("./Model/", 'Smile_Transformer_esm_GraphCAI_EC_contrastive_500.pkl'))
        self.aupr_max = AUPRC
...
def train_one_epoch(model, data_loader, epoch, class_center, data_path):
    ...
    for i, data in enumerate(data_loader):
        ...
        # evo_feature is read from the H5 file generated by Prot5
        ...
        node_features = torch.squeeze(node_features)  # Remove dimensions with size 1 from the tensor
        ...
        try:
            ...
            # Print the sequence name and features
        ...
        # ec_fea = ec_fea.squeeze(0)  # shape: [1024]; manually added for dimension adjustment
...
def train_model(train_dataframe, valid_dataframe, data_path, ec2pro):
    ...
    valid_loader = DataLoader(dataset=ProDataset(valid_dataframe, data_path), batch_size=BATCH_SIZE, shuffle=True,
                              num_workers=2)
    if torch.cuda.is_available():
        torch.cuda.set_device(4)  # Specify the GPU to use; indexing starts from 0
    ...
    for epoch in range(NUMBER_EPOCHS):
        ...
        if epoch % 10 == 0:
            ...
            class_center = get_cluster_center(ec2pro, data_path, epoch)  # Provide a class-center feature for each EC class
        ...
        if epoch > 200:
            ...
            # early_stopping(result_valid['AUPRC'], model)   Temporarily disabled for testing
        ...
        if (epoch + 1) % 20 == 0:
            ...
            torch.save(model.state_dict(), os.path.join("./Model/", f'Smile_Transformer_esm_GraphCAI_EC_contrastive_500_{epoch}.pkl'))


def main():
    # Load PDB IDs for training
    ...
    # Load enzyme function labels (EC1)
