...
from EC_contrastive_transformer_esm import *
...
class EnzDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Step 1: Load features for all residues
...
def evaluate(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            ...
            # print(f"graphs shape: {graphs.shape}")
            # print(f"evo_feature shape: {evo_feature.shape}")

            y_pred, enzfeas = model(node_features, graphs, evo_feature, inter_feature)

            # Print the dimension of prediction results
...
def test(test_dataframe, data_path):
    test_loader = DataLoader(dataset=EnzDataset(test_dataframe, data_path), batch_size=BATCH_SIZE, shuffle=True,
                             num_workers=1)

    # model_name = 'Transformer_esm_GraphCAI_EC_contrastive_500_199.pkl'     # Used for NN and PC
    model_name = 'Transformer_esm_GraphCAI_EC_contrastive_500_239.pkl'     # Used for one superfamily dataset; another one also uses a checkpoint after 200 epochs
    model = GraphCAI(NLAYER, INPUT_DIM, HIDDEN_DIM, NUM_CLASSES, DROPOUT)

    if torch.cuda.is_available():
        torch.cuda.set_device(1)  # Specify the GPU to use; indexing starts from 0
        print("Using GPU:", torch.cuda.current_device())

    if torch.cuda.is_available():
        model.cuda()

    model.load_state_dict(torch.load(Model_Path + model_name, map_location='cuda:2'))
...
def main():
    ...
    dataset = ["HA_superfamily"]
    index = 0

    # Load PDB IDs for testing
    ...
    prot_seq = {}
    prot_anno = {}
    f = open("./Dataset/HA_superfamily/HA_superfamily_enzyme_label.txt", "r")
    data = f.readlines()
    ...
    data_dir = "./Dataset/HA_superfamily/"
