...
# HHBLITS = "/path/to/hhblits"  # HH-SUITE software folder path for features extraction
# UR90 = "/path/to/uniref90_database"  # database for PSSM path # PSIBLAST DATABASE folder path for feature extraction
# HHDB = "/path/to/uniclust30_database"
# dssp = "/path/to/mkdssp"
PSIBLAST = "/path/to/psiblast"
HHBLITS = "/path/to/hhblits"  # HH-SUITE software folder path for features extraction
UR90 = "/path/to/uniref90_database"  # database for PSSM path # PSIBLAST DATABASE folder path for feature extraction
...
def def_atom_features():
    ...
    atom_features = {'A': A, 'V': V, 'F': F, 'P': P, 'L': L, 'I': I, 'R': R, 'D': D, 'E': E, 'S': S, 'T': T, 'C': C,
                     'N': N, 'Q': Q, 'H': H, 'K': K, 'Y': Y, 'M': M, 'W': W, 'G': G}
...
def write_contact_map(prot, prot2seq, data_path):
    # Initialize the error list on the first call
    if not hasattr(write_contact_map, "error_list"):
        write_contact_map.error_list = []
    ...
    if not os.path.exists(data_path + 'contact_map/{}.npy'.format(prot)):
        ...
        try:
            ...
            print(e)
            print("{}.npy生成错误".format(prot))
            write_contact_map.error_list.append(prot)

    # This assumes the function is called repeatedly in a loop.
    # Print the current accumulated error information after each call if needed:
    print(f"当前累计错误数量: {len(write_contact_map.error_list)}")
    if len(write_contact_map.error_list) > 0:
        print("错误文件名列表：", write_contact_map.error_list)
...
def cal_PSSM(seq_list, pssm_dir):
    processed_dir = os.path.join(pssm_dir, 'processed_pssm')
    os.makedirs(processed_dir, exist_ok=True)

    for seqid in seq_list:
        print(f"Processing: {seqid}")
        PDBID, Chain = seqid.split('-')
        pro1 = f"{PDBID}_{Chain}"
        file = f"{seqid}.pssm"
        file1 = f"{pro1}.pssm"

        # Try two naming formats
        if os.path.exists(os.path.join(pssm_dir, file)):
            pssm_file = os.path.join(pssm_dir, file)
        elif os.path.exists(os.path.join(pssm_dir, file1)):
            pssm_file = os.path.join(pssm_dir, file1)
        else:
            print(f"[Error] File not found for {seqid}")
            continue

        with open(pssm_file, 'r') as fin:
            fin_data = fin.readlines()

        # Valid data starts from the fourth line and ends before the last six lines
        pssm_begin_line = 3
        pssm_end_line = len(fin_data) - 6  # Exclude the last six lines

        if pssm_end_line <= pssm_begin_line:
            print(f"[Warning] Skipping {seqid}: Not enough data lines.")
            continue

        feature_list = []
        for i in range(pssm_begin_line, pssm_end_line):
            tokens = fin_data[i].split()
            if len(tokens) < 22:
                print(f"[Warning] Skipping line {i} in {seqid}: not enough columns.")
                continue
            raw_pssm = tokens[2:22]
            line_feature = [1 / (1 + math.exp(-float(val))) for val in raw_pssm]
            feature_list.append(line_feature)

        feature = np.array(feature_list)
        np.save(os.path.join(processed_dir, f"{seqid}.npy"), feature)
...
#             res_pdb_id = int(line[22:26])  # Residue index
#
...
def matched_dssp(sequence_name, ref_seq, PDB_dir, dssp_dir):
    ...
    file_path = PDB_dir + sequence_name + '.pdb'
    with open(file_path, 'r') as pdb_handle:
        sequence = ''
        res_id_list = []
        ...
        for line in pdb_handle:
            if line.startswith('ATOM'):
                ...
                if res_name not in res_dict:
                    continue  # Ignore unknown residues
    ...
    dssp_path = os.path.join(dssp_dir, 'processed_dssp', sequence_name + '.npy')
    print(f"Loading DSSP feature from: {dssp_path}")
    dssp_feature = np.load(dssp_path, allow_pickle=True)

    if len(dssp_feature) == 0:
        raise ValueError(f"[Error] DSSP feature is empty for {sequence_name}")

    data_shape = dssp_feature[0].shape
    print(f"[Debug] dssp_feature.shape = {dssp_feature.shape}")
    print(f"[Debug] Per-residue feature shape: {data_shape}")
    ...
    for aa in align_seq:
        if aa == "-":
            new_dssp.append(np.zeros(data_shape))  # Insert a zero vector
        else:
            if dssp_index >= len(dssp_feature):
                # If DSSP data is insufficient, replace it with a zero vector
                print(f"[Warning] DSSP feature ran out at index {dssp_index}, inserting zeros.")
                new_dssp.append(np.zeros(data_shape))
            else:
                new_dssp.append(dssp_feature[dssp_index])
                dssp_index += 1
    ...
    save_dir = os.path.join(dssp_dir, 'matched_dssp')
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f'{sequence_name}.npy')
    np.save(save_path, matched_dssp)
    print(f"[Success] Saved matched DSSP for {sequence_name} to {save_path}")
...
def Multi_process_PSSM_generation(PDB_id, data_path):
    ...
    #     os.system(
    #         "{0} -db {1} -num_iterations 3 -num_alignments 1 -num_threads 2 -query {3}fasta/{2}.fa -out {3}fasta/{2}.bla -out_ascii_pssm {3}pssm/{2}.pssm".format(
    #             PSIBLAST, UR90, PDB_id, data_path))
    # if os.path.exists(data_path + 'hhm/{}.hhm'.format(PDB_id)) == False:
    #     print("starting to generate hhm", PDB_id)
    #     os.system(
    #         "{0} -i {2}fasta/{1}.fa -ohhm {2}hhm/{1}.hhm -oa3m {2}fasta/{1}.a3m -d {3} -v 0 -maxres 40000 -cpu 6 -Z 0 -o {2}fasta/{1}.hhr".format(
    #             HHBLITS, PDB_id, data_path, HHDB))
...
def get_pdb_DF(prot, data_path):
    ...
    file_path = os.path.join(data_path, "PDB/SavechainPDB", prot + '.pdb')
    ...
    Relative_atomic_mass = {
        'H': 1, 'C': 12, 'O': 16, 'N': 14, 'S': 32, 'FE': 56, 'P': 31, 'BR': 80, 'F': 19, 'CO': 59, 'V': 51,
    ...
    for line in lines:
        if line.startswith('ATOM'):
            ...
            is_sidechain = 1 if line[12:16].strip() not in ['N', 'CA', 'C', 'O', 'H'] else 0
            res = res_dict[line[17:20]]
            atom = line[12:16].strip()
            atom_fea = atom_fea_dict.get(res, {}).get(atom, [0.5, 0.5, 0.5])
    ...
    pdb_res = pd.DataFrame(records)

    save_dir = os.path.join(data_path, 'Atom_feas')
    os.makedirs(save_dir, exist_ok=True)

    with open(os.path.join(save_dir, f'{prot}.csv.pkl'), 'wb') as f:
        pickle.dump({
            'pdb_DF': pdb_res,
...
def cal_atomFea(seqlist, data_path):
    ...
    for seq_id in tqdm(seqlist):
        try:
            ...
            # If there is no atom_type column, use the atom column for filtering
            if 'atom_type' in pdb_res.columns:
                pdb_res = pdb_res[pdb_res['atom_type'] != 'H']
                atom_type = pdb_res['atom_type'].tolist()
            elif 'atom' in pdb_res.columns:
                pdb_res = pdb_res[pdb_res['atom'] != 'H']
                atom_type = pdb_res['atom'].tolist()
            else:
                print(f"[Warning] {seq_id} pdb_res missing 'atom_type' and 'atom' columns, skipping")
                continue
            ...
            for i, type_ in enumerate(atom_type):
                try:
                    atom_vander[i] = atom_vander_dict[type_]
                except:
            ...
            print(f"[Warning] Skipping {seq_id} due to error: {e}")
            continue
...
if __name__ == '__main__':
    ...
    # # 3. Count all n-grams and build word_dict
    # ngram_count = defaultdict(int)
    # ngram_size=3
    # ngram_dir = os.path.join(data_dir, "ngram")
    # for pro in protein_list:
    #     seq = prot_seq.get(pro)
    #     if seq:
    #         for i in range(len(seq) - ngram_size + 1):
    #             gram = seq[i:i + ngram_size]
    #             ngram_count[gram] += 1
    #
    # # Assign a unique ID to each n-gram, starting from 1, with 0 reserved for padding
    # word_dict = {gram: idx + 1 for idx, gram in enumerate(ngram_count)}
    # np.save(os.path.join(ngram_dir, "word_dict_3.npy"), word_dict)
    # print(f"Saved word_dict with {len(word_dict)} entries.")
    #
    # # 4. Generate the n-gram index array for each sequence
    # for pro in protein_list:
    #     seq = prot_seq.get(pro)
    #     if not seq:
    #         continue
    ...
    #     for i in range(len(seq) - ngram_size + 1):
    #         gram = seq[i:i + ngram_size]
    #         idx = word_dict.get(gram, 0)  # Use 0 for unknown n-grams
    #         word_ids.append(idx)
    #     word_ids = np.array(word_ids, dtype=np.int64)
    #     np.save(os.path.join(ngram_dir, pro + ".npy"), word_ids)

    # ---------write enzyme fasta--------------
    # write_all_fasta(data_dir, protein_list, prot_seq)  # Write all protein sequences into a single FASTA file
    # Write_single_fasta(data_dir + "training_label.txt", data_dir)  # Generate a separate FASTA file for each protein

    # ---------write specific chain PDB
    # download_pdb(protein_list, data_dir + "PDB/")  # Download compressed files
    SaveChainPDB(protein_list, data_dir + "PDB/")  # Decompress and split into different PDB files by chain

    # Process CA contact maps: save the C-alpha distances between residues in each PDB file as NPY files
    # nprocs = 20  # Set the maximum number of processes
    # import multiprocessing

    # nprocs = np.minimum(nprocs, multiprocessing.cpu_count())  # Limit the number of processes to the number of CPU cores
    # if nprocs > 4:  # Use multiprocessing if the available process count is greater than 4
    #     pool = multiprocessing.Pool(processes=nprocs)
    #     # Use partial to wrap write_contact_map and its parameters
        pool.map(partial(write_contact_map, prot2seq=prot_seq, data_path=data_dir), protein_list)
    # else:  # Otherwise, use a single process
    # for prot in protein_list:
    ...
    # # PSSM represents conservation features; higher conservation indicates greater stability. HHM predicts the probability that a position is a certain residue. DSSP represents secondary structure.
    ...
    #
    #
    # # Extract detailed information for each atom from the PDB file, including atom type, residue name, coordinates, B-factor, etc.
    # # Calculate features for each atom, such as charge, number of hydrogen atoms, and whether it is in an aromatic ring, then save them to a pickle file.
    # if nprocs > 4:
    #     pool = multiprocessing.Pool(processes=nprocs)
    ...
    # Process features and generate NPY files
    # cal_atomFea(protein_list, data_dir)

    # for seqid in protein_list:
    #     matched_atomfea(seqid, prot_seq[seqid], data_dir)  # Process the features generated above
    #     seq_fea_generate(seqid, prot_seq[seqid], data_dir)  # Map the sequence to predefined feature vectors
